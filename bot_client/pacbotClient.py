...
# Elec code for controlling Robot
...
class PacbotClient:
    ...
    async def run(self) -> None:
        ...
        await self.connect()
        await self.decisionModule.create_server()
        ...
        try:  # Try receiving messages indefinitely
            ...
            if self._socketOpen:
                ...
                await asyncio.gather(
                    self.receiveLoop(), self.decisionModule.decisionLoop(
                    ), self.decisionModule.pi_server.serve_forever(),
                )
        finally:
            # Disconnect once the connection is over
